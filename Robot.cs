...
namespace Karesz
{
    partial class Form1
    {
        class Robot : Test
        {
            ...
            { }
            public Robot(string adottnév, Bitmap[] képkészlet, int fekete_db, int piros_db, int zöld_db, int kék_db, int hó_db, int x, int y, int f, bool uh_engedélyezve = true, bool szuh_engedélyezve = true) :
                            this(adottnév, képkészlet, new int[] { fekete_db, piros_db, zöld_db, kék_db, hó_db }, new Vektor(x, y), new Vektor(f), uh_engedélyezve, szuh_engedélyezve)
            { }
            public Robot(string adottnév, int fekete_db, int piros_db, int zöld_db, int kék_db, int hó_db, int x, int y, int f, bool uh_engedélyezve = true, bool szuh_engedélyezve = true) :
                            this(adottnév, new int[] { fekete_db, piros_db, zöld_db, kék_db, hó_db }, new Vektor(x, y), new Vektor(f), uh_engedélyezve, szuh_engedélyezve)
            { }
